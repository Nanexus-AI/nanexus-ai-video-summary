...
阶段 9 之后采用顺序 Gate：先完成部署 Closeout，再做只读影子对照；随后才可单独授权 MQTT Cutover Rehearsal 和 Legacy Retirement；License/SBOM 可提前审计，但开源发布始终需要最后的人工授权。任何 Gate 的完成都不自动授权下一 Gate。
